...
class FinalChallengeViewController : UIViewController {
    ...
    override func loadView() {///Carega/Cria a view
        ...
        label.text = "Hello World! final challenge"
    ...
    override func viewDidLoad() {///chamada quando a view, criada via storyboard, termina de ser carregada
        refreshButton.addTarget(self, action: #selector(FinalChallengeViewController.touchedRefreshButton), for: .touchUpInside)
        proximaFaseButton.addTarget(self, action: #selector(FinalChallengeViewController.touchedProximaFaseButton), for: .touchUpInside)
    }
}
...
PlaygroundPage.current.liveView = FinalChallengeViewController()
let vc = FinalChallengeViewController(screenType: .mac, isPortrait: true)
